    ...
    *{
    ...
    <div>
        ...
        <div style="display: inline-block;max-width:49%;width:49%;text-align:right;">
            <h2>Invoice</h2>
            <p><i>Order <strong>#{{ $invoice->invoice_no }}</strong></i></p>
    ...
    <div>
        <div style="display: inline-block;max-width:50%;width:50%">
            ...
            <p>{{ $invoice->customer->telephone }}</p>
        ...
        <div style="display: inline-block;max-width:49%;width:49%;text-align:right;vertical-align: top;">
            ...
            <p>{{ date(\App\Config::where('account_id', Auth::user()->account_id)->first()->date_format, strtotime($invoice->updated_at))}}</p>
        </div>
    </div>

    <hr style="margin:5px 0">

    @if(count(json_decode($invoice->products))>0)
    ...
    @if(count(json_decode($invoice->services))>0)
    ...
    @if(count(json_decode($invoice->costs))>0)
        ...
        <div>
            <table style="border-collapse: collapse;width: 100%;">
                ...
                <tbody>
                    ...
                    @php
                        $costs =  json_decode($invoice->costs);
                    @endphp
