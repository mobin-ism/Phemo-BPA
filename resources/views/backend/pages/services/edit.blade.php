...
<form method="post" id="modal-form" action="{{ route('services.update', $service) }}">
    @csrf
    {{method_field('PATCH')}}
    ...
    <div class="row mb-4">
        <div class="col-lg-6">
            ...
            <div class="form-group m-form__group" id="rate-group">
                <label for="rate">
                    * {{__('web.rate')}}
                </label>
                <input type="text" class="form-control m-input m-input--pill" id="rate" name="rate"
                    value="{{$service->rate}}">
            </div>
            <div class="form-group m-form__group">
                ...
                <select class="form-control m-bootstrap-select m-bootstrap-select--pill m_selectpicker"
                    ...
                    <option value="{{$uom->id}}" <?php if ($service->unit_of_measure_id == $uom->id) echo 'selected'; ?>>{{$uom->name}}</option>
                    @endforeach
        ...
        <div class="col-lg-6">
            <div class="form-group m-form__group" id="">
                ...
                <textarea class="form-control m-input m-input--pill" id="description" name="description" rows="5">{{$service->description}}</textarea>
            </div>
        </div>
    </div>
</form>
