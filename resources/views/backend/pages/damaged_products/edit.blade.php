...
<form method="post" id="modal-form" action="{{ route('damaged_products.update', $damaged_product->id) }}">
    @csrf
    {{method_field('PATCH')}}
    ...
    <div class="row mb-4">
        <div class="col-lg-6">
            <div class="form-group m-form__group" id="product_id-group">
                ...
                <select class="form-control m-bootstrap-select m-bootstrap-select--pill m_selectpicker"
                    ...
                    <option value="{{$product->id}}" <?php if ($product->id == $damaged_product->product_id) echo 'selected';?>>
                        {{$product->name}}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-group m-form__group" id="quantity-group">
                ...
                <input type="text" class="form-control m-input m-input--pill" id="quantity" name="quantity"
                    value="{{$damaged_product->quantity}}">
            </div>
            <div class="form-group m-form__group" id="date-group">
                ...
                <input type="text" class="form-control m-input m-input--pill m_datepicker_1" name="date" 
                    id="date" readonly value="{{get_formatted_date_from_timestamp($damaged_product->date)}}">
            </div>
        ...
        <div class="col-lg-6">
            <div class="form-group m-form__group" id="notes-group">
                ...
                <textarea class="form-control m-input m-input--pill" id="notes" name="notes" rows="5">{{$damaged_product->notes}}</textarea>
            </div>
        </div>
    </div>
</form>
